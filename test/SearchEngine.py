...
class SearchEngine:
    ...
    def get_search_filter_selection(self, header_msg = '==== ITEMS SUGGESTED ====', sub_head_msg = '', main_menu = False, print_permission = True):
        ...
        print("Search for an item and press ENTER\n")
    ...
    ### Foncion de transition
    def find_input_state(self, input_):
        list_inputs = []
        for char in input_:
            list_inputs.append(char)

        ## TYPE ##
        if len(list_inputs) == 1 and ord(list_inputs[0]) >= 65 and ord(list_inputs[0]) <= 67:
            return "type_state"

    def search(self, input_):
        if(input_ != '1' or input_ != '2' or input_ != '3' or input_ != '4' or input_ != '5'or input_ != '6'or input_ != '7'or input_ != '8'or input_ != '9'or input_ != '0'):
            object_to_find = Object('','','')
            
            ### OBJECT STATES ###
            ### INITIAL STATE ### the initial state is the empty (start)

            ### The next state depends on the input ###
            next_state = self.find_input_state(input_)
            object_not_found = True
            while(object_not_found) :
                
                ### TYPE STATE ### 
                if next_state == "type_state":
                    print("INSIDE WHILE TYPE STATE")
                    object_not_found = False

                ### ID STATE ### 
                elif next_state == "id_state":
                    pass

                ### NAME STATE ###
                elif next_state == "name_state":
                    pass
